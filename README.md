...
- **AI 分域设置** — 可为词池生成（POOL）和 OCR 识别（OCR）配置独立 AI 模型；未开启时自动回退到主设置
...
| AI | Anthropic Claude API / OpenAI 兼容 API（自动整理、OCR 提取、句子分析、词池生成、批量导入） |
...
11. **分析句子** — 在阅读器中长按任意句子，AI 自动分析翻译、语法和关键词
12. **分域设置**（可选）— 在设置页开启「词池 AI」或「OCR AI」，为不同场景配置独立的 AI 模型
