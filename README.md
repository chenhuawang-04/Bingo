...
- **题型全覆盖** — 阅读理解、完形填空、翻译（英语一/英语二）、写作（小作文/大作文）、段落排序、句子插入、评论观点匹配、小标题匹配、信息匹配
...
- **写作题处理** — 写作题范文使用搜索模型联网查找真实范文与链接；用户提交作文后支持 OCR 识别与 AI 批阅
- **翻译评分** — 翻译题支持 AI 评分与参考译文展示
- **文章出题** — 在文章阅读页一键出题，选择题型与题卷名，主模型自动生成题干（不含答案与解析）
- **分域 AI 设置** — 新增搜索模型（SEARCH）独立配置，题库扫描复用 OCR 模型
...
- **文章适配度评分** — 快速模型评估文章是否适合出题，给出 0–100 评分与理由，支持手动重新评估与按评分排序

### 在线阅读（多来源）
- **来源选择** — 支持 Guardian 与 The Atlantic 等阅读源，统一在线阅读入口
- **栏目浏览** — 按栏目分组浏览热门文章
...
### 设置
- **API 配置** — 支持 Anthropic 和 OpenAI 兼容两种提供商，允许添加多个自定义提供商（唯一名、Base URL、Key）
- **AI 分域设置** — 各域（POOL / OCR / ARTICLE / MAIN / SEARCH）可独立绑定提供商与模型；支持手动输入模型与从 `/models` 拉取
...
17. **分域设置**（可选）— 在设置页开启词池/ OCR / 文章 / 搜索 AI，配置独立模型
